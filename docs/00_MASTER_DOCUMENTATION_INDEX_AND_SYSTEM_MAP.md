...
3. **[Agentic Sacred Covenant & Kirk LaSalle's 10 Laws of AI Supremacy](../AGENTIC_SACRED_COVENANT.md)**  
   *The foundational human-AI partnership agreement, permanent active directives, and ethical governance covenant.*
4. **[Agentic Prime Directive Specification](../AGENTIC_PRIME_DIRECTIVE.md)**  
   *Operational nervous system, cryptographically enforced governance plane, and tri-model cognitive orchestration.*
5. **[UAH Standard Specification (v1.0.0)](UAH_UNREAL_AGENTIC_HARNESS_STANDARD_SPECIFICATION.md)**  
   *The official multi-generational agentic harness specification covering Engine Abstraction (EAL), Universal Command Protocol (UCIP), Agent Tooling (ATFS), Multi-Modal Perception (MPVT), and SAC-POP Post Office Interop.*
6. **[System Architecture & Telemetry Specification](ARCHITECTURE.md)**  
   *Comprehensive architecture breakdown: Win32 message queues, IPC controllers, WebSocket API servers, Tkinter cockpits, and telemetry.*
7. **[Market Landscape & Competitive Intelligence Audit](MARKET_LANDSCAPE_AND_COMPETITIVE_ANALYSIS.md)**  
   *Comparative study of level design tooling, Copilot paradigms, and autonomous agent frameworks across Unreal Engine 1 through 5.*
8. **[API Reference](API_REFERENCE.md)**  
   *Complete REST, WebSocket, and Win32 IPC command endpoint reference.*
...
## 🛠️ Tooling & Utility Scripts

All UnrealScript and package utilities are consolidated under the [`tools/`](../tools/) directory:

| Script | Type | Description |
| :--- | :--- | :--- |
| [`Compile-UnrealScript.ps1`](../tools/Compile-UnrealScript.ps1) | PowerShell | Automated `UCC.exe make` compiler with automatic safety backups. |
| [`Extract-UnrealScript.ps1`](../tools/Extract-UnrealScript.ps1) | PowerShell | Automated `UCC.exe batchexport` tool to unpack `.uc` source trees from binary `.u` packages. |
| [`Extract_All_Scripts.bat`](../tools/Extract_All_Scripts.bat) | Batch | One-click batch wrapper to decompile all base engine and mod scripts. |
...
- **`architecture.html`**: Full 5-tier architecture, IPC communication, and Win32 automation guide.
...
- **`css/style.css`**: Luxury Unreal Cyber-HUD theme with dynamic multi-theme engine (Liandri Gold, Na Pali Sanctuary, UE5 Nanite, 1998 Retro, Skaarj Orbital).
- **`js/main.js`**: Interactive 60fps vector constellation canvas, card spotlight tracking, and CSG generator.
